...
** [[https://leetcode.cn/problems/maximum-erasure-value/description/][1695. 删除子数组的最大得分]]
...
(define (maximum-unique-subarray nums)
  (let ((len (vector-length nums))
        ...
        (ans 0))
    (while (< right len)
      (let ((current (vector-ref nums right)))
        (while (hash-ref record current)
          (hash-remove! record (vector-ref nums left))
          (set! sum (- sum (vector-ref nums left)))
          (set! left (1+ left)))
        (hash-set! record current #t)
        (set! sum (+ sum current))
        (set! ans (max ans sum))
        (set! right (1+ right))))
    ans))
...
#+BEGIN_SRC scheme :session
(maximum-unique-subarray #(4 2 4 5 6))
#+END_SRC
...
#+BEGIN_SRC scheme :session
(maximum-unique-subarray #(5 2 1 2 5 2 1 2 5))
#+END_SRC

#+RESULTS:
: 8

** [[https://leetcode.cn/problems/length-of-longest-subarray-with-at-most-k-frequency/description/][2958. 最多 K 个重复元素的最长子数组]]

*** 思路
窗口內元素最多重複k次，右指針遇到重複，左指針應該移到將計數減一
...
(define (max-subarray-length nums k)
  (let ((len (vector-length nums))
    ...
    (let loop ((left 0)
      ...
      (if (< right len)
          (let ((current (vector-ref nums right)))
            (if (eq? (hash-ref record current 0) k)
                (let ((left-num (vector-ref nums left)))
                  (hash-set! record left-num (- (hash-ref record left-num 0) 1))
                  (loop (+ left 1)
                        right
                        ans))
                (begin
                  (hash-set! record current (+ (hash-ref record current 0) 1))
                  (loop left
                        (+ right 1)
                        (max ans (+ (- right left) 1))))))
          ans))))
...
#+BEGIN_SRC scheme :session
(max-subarray-length #(1 2 3 1 2 3 1 2) 2)
#+END_SRC
...
*** 思路
無非是將答案更新換成求最小，可以將答案初始化成數組長度加一

*** 代碼

#+BEGIN_SRC scheme :session
(define (min-sub-array-len t nums)
  (let* ((len (vector-length nums))
         (impossible (+ len 1)))
    (let loop ((left 0)
               (right 0)
...
** [[https://leetcode.cn/problems/subarray-product-less-than-k/description/][713. 乘积小于 K 的子数组]]

*** 思路
右指針移動，滿足條件，計數加一，不滿足，左指針移動直到再次滿足
  ...
  (let ((len (vector-length nums)))
    (if (<= k 1)
        ...
        (let ((left 0)
          ...
          (while (< right len)
            (let ((current (vector-ref nums right)))
              (set! product (* product current))
              (while (>= product k)
                (set! product (/ product (vector-ref nums left)))
                (set! left (+ left 1)))
